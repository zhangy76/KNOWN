...
def main(video_path, save_video=False, checkpoint='./data/model.pt'):
    ...
    if save_video:
        ...
        video_writter = cv2.VideoWriter('./output.avi', fourcc, 20.0, (int(config.IMG_RES_detect*3), int(config.IMG_RES_detect)))

    W_frame = int(videoclip.get(cv2.CAP_PROP_FRAME_WIDTH))
    H_frame = int(videoclip.get(cv2.CAP_PROP_FRAME_HEIGHT))
    ...
    while videoclip.isOpened():
        ...
        # crop the original frame to a square image with size ()
        # frame, _, Len_max_ori = crop2expandsquare_zeros(frame, [0, 0, W_frame, H_frame])
        if W_frame>H_frame:
        ...
        if len(detections) < 1 or len(detections[0])==3 or detections[0].shape[0] == 0:
            ...
            if save_video:
                video_writter.write(np.uint8(frame_output*255))
...
parser.add_argument('--video_path', type=str, default='', help='Path to the source video; open the camera if a path is not specified')
...
if __name__ == '__main__':
    ...
    main(args.video_path, args.save)
